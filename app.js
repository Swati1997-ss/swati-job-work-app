(() => {
  ...
  const batchWorkspaceHost=$('companySalesBatchHost'),standaloneBatchScreen=$('screen-stock');
  if(batchWorkspaceHost&&standaloneBatchScreen){
    while(standaloneBatchScreen.firstChild) batchWorkspaceHost.appendChild(standaloneBatchScreen.firstChild);
    standaloneBatchScreen.remove();
  }
  ...
  function companyStock(){
    ...
    const coreBalance=id=>{
      if(!window.SwatiCore?.stockSnapshot) return null;
    ...
    const bagPurchases=purchases.filter(x=>Number(x.kgPerBag||0)>0);
    const bagKg=bagPurchases.reduce((s,x)=>s+Number(x.qtyKg||0),0);
    const bagCount=bagPurchases.reduce((s,x)=>s+Number(x.bagCount||(x.unit==='bag'?x.enteredQty:0)||0),0);
    const avgKgPerBag=bagCount?round2(bagKg/bagCount):0;
    return {
  ...
  function tinLooseLabel(totalKg){
    const kg=Math.max(0,round2(totalKg));
    const tins=Math.floor((kg+.0001)/15);
    const loose=round2(kg-(tins*15));
    return `${tins} ડબા + ${loose} kg`;
  }
  function readyOilLabel(tinCount,totalKg){const tins=Math.max(0,Number(tinCount||0));return `${tins} ડબા + ${round2(Math.max(0,Number(totalKg||0)-tins*15))} kg`;}
  function batchOilLabel(r){return `${Number(r?.tinCount||0)} ડબા + ${round2(r?.looseOilKg ?? Math.max(0,Number(r?.oilKg||0)-Number(r?.tinCount||0)*15))} kg`;}

  function oilQtyInKg(qty,unit){
    const q=Number(qty||0);
    if(unit==='tin') return round2(q*Number(settings.tinKg||15));
    if(unit==='ton') return round2(q*1000);
    if(unit==='mann') return round2(q*20);
    if(unit==='litre') return round2(q*.92);
    return round2(q);
  }

  function linkedJobWorkOilIssue(id){
    return (window.SwatiCore?.list('sales')||[]).find(s=>s.context?.sourceModule==='jobwork_oil_stock_alpha41'&&s.context?.notes===id);
  }

  function syncJobWorkOilStock(r){
    if(!window.SwatiCore) return;
    const existing=linkedJobWorkOilIssue(r.id);
    const kg=r.oilSale?.enabled?oilQtyInKg(r.oilSale.qty,r.oilSale.unit):0;
  ...
  function migrateRow(r){
    ...
    const materialLines=Array.isArray(r?.incoming?.materials)?r.incoming.materials:[];
    const storedMaterial=(type)=>materialLines.find(x=>x?.type===type)||{};
    const legacyPodsKg=Number(r?.incoming?.podsKg ?? r?.incoming?.singGoglaKg ?? legacyGroundnut ?? 0);
    const legacyGrainsKg=Number(r?.incoming?.grainsKg ?? r?.incoming?.danaFalaKg ?? 0);
    const legacySplitsKg=Number(r?.incoming?.splitsKg ?? 0);
    const pods=storedMaterial('pods'),grains=storedMaterial('grains'),splits=storedMaterial('splits');
    const incoming = {
      podsKg:Number(pods.kg ?? legacyPodsKg),
      grainsKg:Number(grains.kg ?? legacyGrainsKg),
      splitsKg:Number(splits.kg ?? legacySplitsKg),
      podsQty:Number(pods.enteredQty ?? r?.incoming?.podsQty ?? legacyPodsKg),
      podsUnit:pods.unit||r?.incoming?.podsUnit||'kg',
      grainsQty:Number(grains.enteredQty ?? r?.incoming?.grainsQty ?? legacyGrainsKg),
      grainsUnit:grains.unit||r?.incoming?.grainsUnit||'kg',
      splitsQty:Number(splits.enteredQty ?? r?.incoming?.splitsQty ?? legacySplitsKg),
      splitsUnit:splits.unit||r?.incoming?.splitsUnit||'kg'
    };
    incoming.materials=[
      {type:'pods',label:'શીંગ / Pods',enteredQty:incoming.podsQty,unit:incoming.podsUnit,kg:incoming.podsKg},
      {type:'grains',label:'દાણા / Grains',enteredQty:incoming.grainsQty,unit:incoming.grainsUnit,kg:incoming.grainsKg},
      {type:'splits',label:'ફાડા / Splits',enteredQty:incoming.splitsQty,unit:incoming.splitsUnit,kg:incoming.splitsKg}
    ];
    incoming.singGoglaKg=incoming.podsKg;
    incoming.danaFalaKg=round2(incoming.grainsKg+incoming.splitsKg);
    ...
    return {
      ...r,
      incoming,
      groundnutKg: round2(incoming.podsKg + incoming.grainsKg + incoming.splitsKg),
      ...
      outputs:{lossKg:Number(r?.outputs?.lossKg||0),accountedKg:Number(r?.outputs?.accountedKg||0)},
  ...
  function refreshBatchMaterials(){
    ...
    select.innerHTML='<option value="મગફળી ખરીદી → ખોળ વેચાણ">મગફળી ખરીદી → ખોળ વેચાણ</option>';
    select.value='મગફળી ખરીદી → ખોળ વેચાણ';
  }

  function calculateBatch(){
    if(!$('batchMaterial')) return null;
    const from=$('batchFromDate')?.value||'',to=$('batchToDate')?.value||'';
    const purchases=getCompanyPurchases().filter(x=>(!from||x.date>=from)&&(!to||x.date<=to));
    const kholSales=getCompanySales().filter(x=>x.product==='khol'&&(!from||x.date>=from)&&(!to||x.date<=to));
    const rawKg=round2(purchases.reduce((s,x)=>s+Number(x.qtyKg||0),0));
    const qty=round2(kholSales.reduce((s,x)=>s+Number(x.kg||0),0));
    const purchaseCost=round2(purchases.reduce((s,x)=>s+Number(x.total||0),0));
    const extra=round2(num('batchProcessingCost')),sale=round2(kholSales.reduce((s,x)=>s+Number(x.total||0),0)),totalCost=round2(purchaseCost+extra),profit=round2(sale-totalCost);
    const allocation={allocations:[],cost:purchaseCost,unfilled:0},lots=[];
    $('batchMaterial').value='મગફળી ખરીદી → ખોળ વેચાણ';$('batchQty').value=qty;$('batchQtyUnit').value='kg';$('batchSaleAmount').value=sale;if(to)$('batchSaleDate').value=to;
    $('batchAvailability').textContent=`${purchases.length} મગફળી ખરીદી • ${rawKg} kg • ${kholSales.length} ખોળ વેચાણ • ${qty} kg`;
    ...
    return {lots,available:rawKg,rawKg,qty,allocation,purchaseCost,extra,totalCost,sale,profit,purchaseCount:purchases.length,saleCount:kholSales.length};
  }
  ...
  function saveBatch(e){
    ...
    if(!c.purchaseCount&&!c.saleCount){toast('આ સમયગાળામાં ખરીદી અથવા ખોળ વેચાણ નથી');return;}
    const shortage=0;
    const rows=getBatches(); const old=editingBatchId?rows.find(b=>b.id===editingBatchId):null;
    const rec={id:editingBatchId||crypto.randomUUID(),batchNo:$('batchNo').value,material:'મગફળી ખરીદી → ખોળ વેચાણ',fromDate:$('batchFromDate').value,toDate:$('batchToDate').value,saleDate:$('batchToDate').value,enteredQty:c.qty,qtyUnit:'kg',qty:c.qty,rawInputKg:c.rawKg,purchaseCount:c.purchaseCount,saleCount:c.saleCount,allocations:[],stockShortageKg:shortage,purchaseCost:c.purchaseCost,processingCost:c.extra,totalCost:c.totalCost,saleAmount:c.sale,profit:c.profit,note:$('batchNote').value.trim(),createdBy:old?.createdBy||currentOperator(),updatedBy:currentOperator(),deviceId:old?.deviceId||deviceId(),updatedDeviceId:deviceId(),createdAt:old?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};
  ...
  function jobMaterialKg(qty,unit){ return round2(Number(qty||0)*(unit==='mann'?20:1)); }
  function jobMaterialEntry(type){
    const ids={pods:['peanutPodsQty','peanutPodsUnit','શીંગ / Pods'],grains:['peanutGrainsQty','peanutGrainsUnit','દાણા / Grains'],splits:['peanutSplitsQty','peanutSplitsUnit','ફાડા / Splits']}[type];
    const enteredQty=num(ids[0]),unit=$(ids[1]).value||'kg';
    return {type,label:ids[2],enteredQty,unit,kg:jobMaterialKg(enteredQty,unit)};
  }
  function materialQuantityLabel(incoming,type){
    const line=(incoming?.materials||[]).find(x=>x.type===type);
    const kg=Number(line?.kg ?? incoming?.[`${type}Kg`] ?? 0),qty=Number(line?.enteredQty ?? incoming?.[`${type}Qty`] ?? kg),unit=line?.unit||incoming?.[`${type}Unit`]||'kg';
    if(!kg)return '—';
    return unit==='mann'?`${qty} મણ (${kg} કિલો)`:`${kg} કિલો`;
  }
  ...
  function calculate(){
    ...
    const customerMaterials=['pods','grains','splits'].map(jobMaterialEntry);
    const inputKg=round2(customerMaterials.reduce((sum,x)=>sum+x.kg,0));
    ...
    const accountedKg=round2(totalOilKg+kholProducedKg);
    ...
    $('retainedSummary').textContent=`મિલે ખરીદેલ ખોળ: ${kholPurchaseKg} કિલો`;
    const warning=[];
    ...
    return { tins, extraKg, totalOilKg:round2(totalOilKg), exactPerKg, job, khol, kholProducedKg, kholPurchaseKg, kholCustomerKg, lossKg, accountedKg, inputKg, customerMaterials, customerTinQty, tinSale, oilSale, sales, receivable, payable, net, initialPaid, effectivePaid, remaining };
  }
  ...
  function currentRecord(){
    ...
    return {
      ...
      incoming: {
        materials:c.customerMaterials,
        podsKg:c.customerMaterials[0].kg,podsQty:c.customerMaterials[0].enteredQty,podsUnit:c.customerMaterials[0].unit,
        grainsKg:c.customerMaterials[1].kg,grainsQty:c.customerMaterials[1].enteredQty,grainsUnit:c.customerMaterials[1].unit,
        splitsKg:c.customerMaterials[2].kg,splitsQty:c.customerMaterials[2].enteredQty,splitsUnit:c.customerMaterials[2].unit,
        singGoglaKg:c.customerMaterials[0].kg,
        danaFalaKg:round2(c.customerMaterials[1].kg+c.customerMaterials[2].kg)
      },
      groundnutKg:c.inputKg,
      ...
      outputs:{lossKg:c.lossKg,accountedKg:c.accountedKg},
  ...
  function fillPreview(r){
    $('pPeanutPods').textContent=materialQuantityLabel(r.incoming,'pods');
    $('pPeanutGrains').textContent=materialQuantityLabel(r.incoming,'grains');
    $('pPeanutSplits').textContent=materialQuantityLabel(r.incoming,'splits');
  ...
  function resetForm(){ hideOilAudit();
    ...
    ['peanutPodsQty','peanutGrainsQty','peanutSplitsQty'].forEach(id=>$(id).value=0);
    ['peanutPodsUnit','peanutGrainsUnit','peanutSplitsUnit'].forEach(id=>$(id).value='kg');
    $('oilTins').value = 0;
    $('oilExtraKg').value = 0;
    $('kholKg').value = 0;
  ...
  function saveRecord(e){
    ...
    const requestedOilKg=r.oilSale?.enabled?oilQtyInKg(r.oilSale.qty,r.oilSale.unit):0;
    ...
    if(r.outputs.accountedKg>r.groundnutKg+.01){toast('તેલ + ખોળ, ગ્રાહક લાવેલ કુલ માલ કરતાં વધુ ન હોઈ શકે');return;}
  ...
  function ownerTodayPurchaseTotal(){
    if(!window.SwatiCore?.list) return 0;
    const today=ownerTodayISO();
    return round2((window.SwatiCore.list('purchases')||[]).filter(r=>r.date===today).reduce((s,r)=>s+Number(r.amount||0),0));
  }

  function ownerTodayExpenseTotal(){
    if(!window.SwatiCore?.list) return 0;
    const today=ownerTodayISO();
    return round2((window.SwatiCore.list('expenses')||[]).filter(r=>r.date===today).reduce((s,r)=>s+Number(r.amount||0),0));
  }
  ...
  function rawPurchaseCalc(syncPaid=false){
    const enteredQty=Number($('rawQtyKg')?.value||0),unit=$('rawQtyUnit')?.value||'kg',kgPerBag=Number($('rawKgPerBag')?.value||0),rate=Number($('rawRateKg')?.value||0);
    const qtyKg=unit==='mann'?round2(enteredQty*20):unit==='bag'?round2(enteredQty*kgPerBag):round2(enteredQty);
    const materialTotal=round2(enteredQty*rate);
    const transport=Number($('rawTransportCost')?.value||0),broker=Number($('rawBrokerCost')?.value||0),other=Number($('rawOtherCost')?.value||0);
    const total=round2(materialTotal+transport+broker+other);
    if(syncPaid && $('rawPaid')) $('rawPaid').value=total||0;
    const paid=Number($('rawPaid')?.value||0),out=round2(Math.max(0,total-paid));
    const landedPerKg=qtyKg?round2(total/qtyKg):0;
    if($('rawMaterialTotal')) $('rawMaterialTotal').textContent=money(materialTotal);
    ...
    if($('rawQuantitySummary')) $('rawQuantitySummary').textContent=`${qtyKg} kg • ${round2(qtyKg/20)} મણ${unit==='bag'?` • ${enteredQty} ગુણી`:''}`;
    if($('rawCostPerMann')) $('rawCostPerMann').textContent=money(landedPerKg*20);
    if($('rawCostPerBag')) $('rawCostPerBag').textContent=kgPerBag?money(landedPerKg*kgPerBag):'—';
    return {enteredQty,unit,kgPerBag,qtyKg,materialTotal,transport,broker,other,total,landedPerKg,out};
  }

  function productionCalc(){
    const input=oilQtyInKg($('productionInputKg')?.value,$('productionInputUnit')?.value||'kg');
    const tinCount=Math.max(0,Math.floor(Number($('productionTinCount')?.value||0)));
    const looseOilKg=round2(Number($('productionOilLooseKg')?.value||0));
    const oil=round2(tinCount*15+looseOilKg);
    const khali=oilQtyInKg($('productionKholKg')?.value,$('productionKholUnit')?.value||'kg');
    const remainder=round2(input-oil-khali);
    const loss=Math.max(0,remainder),balance=Math.min(0,remainder);
    if($('productionLossKg')) $('productionLossKg').value=loss;
    ...
    if($('productionOilEquivalent')) $('productionOilEquivalent').textContent=`${tinCount} ડબા + ${looseOilKg} kg = ${oil} kg`;
    ...
    if(warn){
      ...
      if(balance<0) msgs.push(`Oil અને Khol input કરતાં ${Math.abs(balance)} kg વધારે છે.`);
      warn.textContent=msgs.join(' ');
      warn.hidden=!msgs.length;
    }
    return {input,oil,tinCount,looseOilKg,khali,loss,balance,oilYield,khaliYield};
  }

  function companySaleCalc(){
    const product=$('companySaleProduct')?.value||'oil';
    let unit=$('companySaleUnit')?.value||'kg';
    if(product==='khol'&&unit==='tin'){unit='kg';$('companySaleUnit').value='kg';}
    const tinCount=Number($('companySaleTinCount')?.value||0),enteredQty=Number($('companySaleKg')?.value||0);
    const qty=unit==='tin'?tinCount:enteredQty;
    const kg=unit==='mann'?round2(enteredQty*20):unit==='ton'?round2(enteredQty*1000):round2(enteredQty);
    const oilKg=product==='oil'?(unit==='tin'?round2(tinCount*15):kg):0;
    const rate=Number($('companySaleRate')?.value||0),paid=Number($('companySalePaid')?.value||0);
    const transport=Number($('companySaleTransport')?.value||0),loading=Number($('companySaleLoading')?.value||0),unloading=Number($('companySaleUnloading')?.value||0);
    const extraCost=round2(transport+loading+unloading),total=round2(qty*rate),net=round2(total-extraCost),out=round2(Math.max(0,total-paid));
    if($('companySaleTotal')) $('companySaleTotal').textContent=money(total);
    if($('companySaleExtraCost')) $('companySaleExtraCost').textContent=money(extraCost);
    if($('companySaleNet')) $('companySaleNet').textContent=money(net);
    ...
    return {product,unit,tinCount,enteredQty,kg,oilKg,qty,transport,loading,unloading,extraCost,total,net,out,bad};
  }
  ...
  function renderCompanyProduction(){
    ...
    if($('rawPurchaseList')) $('rawPurchaseList').innerHTML=purchases.map(x=>`<div class="mini-list-row"><span><strong>${escapeHtml(x.supplier||'—')}</strong><small>${escapeHtml(x.date||'')} • ${escapeHtml(x.village||'')} ${x.yard?`• ${escapeHtml(x.yard)}`:''}</small><small>${escapeHtml(x.quality||'')} • Landed ${money(x.landedCostPerMann||Number(x.rateKg||0)*20)}/મણ</small></span><span class="row-actions"><strong>${x.qtyKg} kg • ${round2(Number(x.qtyKg||0)/20)} મણ</strong><small>${x.bagCount?`${x.bagCount} ગુણી • `:''}${money(x.total)}</small><button type="button" class="edit-chip" data-edit-raw-purchase="${escapeAttr(x.id)}">Edit</button></span></div>`).join('')||'<div class="empty">હજુ ખરીદી નથી.</div>';
    ...
    if($('productionBatchList')) $('productionBatchList').innerHTML=batches.map(x=>`<div class="mini-list-row"><span><strong>${escapeHtml(x.batchNo)}</strong><small>${escapeHtml(x.date)} • ${escapeHtml(x.operator||'')}</small></span><span class="row-actions"><strong>${batchOilLabel(x)}</strong><small>${x.inputKg} kg input</small><button type="button" class="edit-chip" data-edit-production-batch="${escapeAttr(x.id)}">Edit</button></span></div>`).join('')||'<div class="empty">હજુ બેચ નથી.</div>';
  }

  function renderCompanyStock(){
    const s=companyStock();
    if($('companyRawStockKg')) $('companyRawStockKg').textContent=`${s.rawAvailable} kg • ${round2(s.rawAvailable/20)} મણ`;
    if($('companyRawStockMeta')) $('companyRawStockMeta').textContent=`આશરે ${s.avgKgPerBag?round2(s.rawAvailable/s.avgKgPerBag):0} ગુણી • ખરીદી ${s.rawPurchased} kg • વપરાશ ${s.rawUsed} kg`;
    if($('companyReadyOilStock')) $('companyReadyOilStock').textContent=readyOilLabel(s.tinsAvailable,s.oilAvailableKg);
    if($('companyReadyOilMeta')) $('companyReadyOilMeta').textContent=`કુલ ${s.oilAvailableKg} kg • પ્રોડક્શન ${s.oilProduced} kg`;
    if($('companyKholStockKg')) $('companyKholStockKg').textContent=`${s.khaliAvailable} kg • ${round2(s.khaliAvailable/20)} મણ`;
    ...
    if($('companyEmptyBagStock')) $('companyEmptyBagStock').textContent=`${balance('oil.packaging.jute_bag')} કોથળા`;

    const rows=getCompanyBatches().slice().sort((a,b)=>String(b.date).localeCompare(String(a.date)));
    if($('companyProductionHistoryBody')) $('companyProductionHistoryBody').innerHTML=rows.map(x=>`<tr><td>${escapeHtml(x.date)}</td><td>${escapeHtml(x.batchNo)}</td><td>${x.inputKg} kg</td><td>${batchOilLabel(x)}<br><small>${x.oilKg} kg total</small></td><td>${x.khaliKg} kg</td><td>${x.lossKg} kg</td><td>${x.tinCount}</td></tr>`).join('')||'<tr><td colspan="7">હજુ ડેટા નથી.</td></tr>';
    renderCompanyRatioPanel();
  }

  function renderCompanyRatioPanel(){
    if(!$('companyRatioPanel')) return;
    const purchases=getCompanyPurchases(),batches=getCompanyBatches();
    const pKg=purchases.reduce((s,x)=>s+Number(x.qtyKg||0),0);
    const expectedOil=pKg?round2(purchases.reduce((s,x)=>s+Number(x.qtyKg||0)*Number(x.expectedOilRatio||0),0)/pKg):0;
    const expectedKhol=pKg?round2(purchases.reduce((s,x)=>s+Number(x.qtyKg||0)*Number(x.expectedKholRatio||0),0)/pKg):0;
    const input=batches.reduce((s,x)=>s+Number(x.inputKg||0),0),oil=batches.reduce((s,x)=>s+Number(x.oilKg||0),0),khol=batches.reduce((s,x)=>s+Number(x.khaliKg||0),0),diff=Math.max(0,input-oil-khol);
    $('ratioExpectedOil').textContent=`${expectedOil}%`;$('ratioExpectedKhol').textContent=`${expectedKhol}%`;
    $('ratioActualOil').textContent=`${input?round2(oil/input*100):0}%`;$('ratioActualKhol').textContent=`${input?round2(khol/input*100):0}%`;$('ratioProcessDiff').textContent=`${input?round2(diff/input*100):0}%`;
    $('ratioBatchList').innerHTML=batches.slice().sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(x=>`<div class="ratio-batch-row"><strong>${escapeHtml(x.batchNo||'Batch')}<br><small>${escapeHtml(x.date||'')}</small></strong><span>Input<br><b>${x.inputKg} kg</b></span><span>Oil<br><b>${x.oilYield||0}%</b></span><span>Khol<br><b>${x.khaliYield||0}%</b></span><span>Difference<br><b>${x.inputKg?round2(Number(x.lossKg||0)/Number(x.inputKg)*100):0}%</b></span></div>`).join('')||'<div class="empty">Ratio માટે production batch જરૂરી છે.</div>';
  }

  function stockMovementLabel(type){
    return ({purchase_in:'ખરીદી',production_in:'ઉત્પાદન',production_consumption:'ઉત્પાદનમાં વપરાશ',sale_out:'વેચાણ',usage_out:'વપરાશ',adjustment_in:'સ્ટોક ઉમેર્યો',adjustment_out:'સ્ટોક ઘટાડ્યો',opening_in:'Opening Stock'})[type]||type||'Stock Entry';
  }

  function renderCompanyStockDetail(itemId){
    if(!window.SwatiCore || !$('companyStockDetail')) return;
    const rows=(window.SwatiCore.list('stockMovements')||[]).filter(r=>r.itemId===itemId).sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))||String(b.createdAt||'').localeCompare(String(a.createdAt||'')));
    const snap=(window.SwatiCore.stockSnapshot()||[]).find(r=>r.itemId===itemId);
    const incoming=round2(rows.filter(r=>r.direction==='in').reduce((s,r)=>s+Number(r.baseQty??r.qty??0),0));
    const outgoing=round2(rows.filter(r=>r.direction!=='in').reduce((s,r)=>s+Number(r.baseQty??r.qty??0),0));
    $('companyStockDetail').hidden=false;
    $('companyStockDetailTitle').textContent=snap?.itemName||rows[0]?.itemName||'Stock વિગત';
    $('companyStockDetailIn').textContent=incoming;
    $('companyStockDetailOut').textContent=outgoing;
    $('companyStockDetailBalance').textContent=round2(incoming-outgoing);
    let detailHtml=rows.map(r=>`<div class="stock-timeline-row ${r.direction==='in'?'in':'out'}"><span class="stock-timeline-dot"></span><span><strong>${stockMovementLabel(r.movementType)} — ${escapeHtml(({raw_material_purchase:'Groundnut Purchase',production_batch:'Company Production',jobwork_khol_purchase:'Job-work Khol Purchase',jobwork_new_tin_sale:'Job-work Tin Sale',company_sale:'Company Sale',packaging_purchase:'Packaging Purchase',grain_waste_output:'Grain Waste Output'})[r.context?.activity]||r.context?.activity||'Entry')}</strong><small>${escapeHtml(r.date||'')} • ${escapeHtml(r.context?.operator||'')} ${r.context?.notes?`• ${escapeHtml(r.context.notes)}`:''}</small></span><b>${r.direction==='in'?'+':'−'}${Number(r.baseQty??r.qty??0)} ${escapeHtml(r.baseUnitName||r.unitName||'')}</b></div>`).join('');
    if(itemId==='oil.raw.groundnut') detailHtml+=getCompanyPurchases().slice().sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(x=>`<div class="stock-purchase-detail"><strong>${escapeHtml(x.supplier||'Supplier')} • ${escapeHtml(x.date||'')}</strong><small>${escapeHtml(x.quality||'Groundnut')} • ${escapeHtml(x.village||'')} ${x.yard?`• ${escapeHtml(x.yard)}`:''}</small><span>${x.qtyKg} kg • ${round2(Number(x.qtyKg||0)/20)} મણ ${x.bagCount?`• ${x.bagCount} ગુણી × ${x.kgPerBag} kg`:''}</span><span>માલ ${money(x.materialTotal||x.total)} + Transport ${money(x.transportCost)} + Broker ${money(x.brokerCost)} + Other ${money(x.otherCost)}</span><b>Landed ${money(x.total)} • ${money(x.landedCostPerMann||Number(x.rateKg||0)*20)}/મણ</b><small>Expected: Oil ${x.expectedOilRatio||0}% • Khol ${x.expectedKholRatio||0}%</small></div>`).join('');
    $('companyStockDetailList').innerHTML=detailHtml||'<div class="empty">આ આઇટમની હજી કોઈ stock entry નથી.</div>';
    $('companyStockDetail').scrollIntoView({behavior:'smooth',block:'start'});
  }

  function renderCompanySales(){
    if($('companySaleDate') && !$('companySaleDate').value) $('companySaleDate').value=todayISO();
    companySaleCalc();
    const allSales=getCompanySales(),product=$('companySaleProduct')?.value||'khol',sales=allSales.filter(x=>(x.product||'oil')===product);
    if($('companySalePartyOptions')) $('companySalePartyOptions').innerHTML=[...new Set(allSales.filter(x=>x.channel==='bulk'&&x.customer).map(x=>x.customer))].sort().map(name=>`<option value="${escapeAttr(name)}"></option>`).join('');
    const rows=sales.slice().sort((a,b)=>String(b.date).localeCompare(String(a.date))).slice(0,10);
    if($('companySalesTitle')) $('companySalesTitle').textContent=product==='oil'?'તાજેતરનું તેલ વેચાણ':'તાજેતરનું ખોળ વેચાણ';
    if($('companySalesCount')) $('companySalesCount').textContent=String(sales.length);
    if($('companySalesList')) $('companySalesList').innerHTML=rows.map(x=>`<div class="mini-list-row"><span><strong>${escapeHtml(x.customer||'—')}</strong><small>${escapeHtml(x.date)} • ${x.channel==='bulk'?'Bulk Party':'Retail'} • ${escapeHtml(x.village||'')}</small></span><span class="row-actions"><strong>${money(x.total)}</strong><small>${x.unit==='mann'?`${x.enteredQty??round2(Number(x.kg||0)/20)} મણ • ${x.kg} kg`:x.unit==='ton'?`${x.enteredQty} ton • ${x.kg} kg`:x.unit==='tin'?`${x.tinCount} ડબા • ${x.oilKg} kg`:`${x.kg} kg`} • ખર્ચ ${money(x.extraCost||0)} • બાકી ${money(x.outstanding)}</small><button type="button" class="edit-chip" data-edit-company-sale="${escapeAttr(x.id)}">Edit / સુધારો</button></span></div>`).join('')||`<div class="empty">હજુ ${product==='oil'?'તેલ':'ખોળ'} વેચાણ નથી.</div>`;
    renderStock();
  }
  ...
  function purchaseCalc(syncPaid=false){
    ...
    if(syncPaid && $('purchasePaid')) $('purchasePaid').value=goods||0;
  ...
  function renderCosting(){
    ...
    if(!$('profitRawKg')?.value && !$('profitOilKg')?.value) useLatestProfitBatch();
    else profitPlannerCalc();
  }

  function profitPlannerCalc(){
    const rawKg=Number($('profitRawKg')?.value||0),rawCost=Number($('profitRawCost')?.value||0),oilKg=Number($('profitOilKg')?.value||0);
    const processCost=Number($('profitProcessCost')?.value||0),kholKg=Number($('profitKholKg')?.value||0),kholRate=Number($('profitKholRate')?.value||0);
    const margin=Number($('profitMargin')?.value||0),sellingRate=Number($('profitSellingRate')?.value||0);
    const kholCredit=round2(kholKg*kholRate),batchCost=round2(rawCost+processCost),netOilCost=round2(Math.max(0,batchCost-kholCredit));
    const oilCostKg=oilKg>0?round2(netOilCost/oilKg):0,recommended=round2(oilCostKg*(1+margin/100));
    const expected=round2((sellingRate*oilKg)+kholCredit-batchCost);
    if($('profitKholCredit')) $('profitKholCredit').textContent=money(kholCredit);
    if($('profitOilCostKg')) $('profitOilCostKg').textContent=money(oilCostKg);
    if($('profitRecommendedRate')) $('profitRecommendedRate').textContent=money(recommended);
    if($('profitExpectedProfit')) $('profitExpectedProfit').textContent=`${expected<0?'Loss ':'Profit '}${money(Math.abs(expected))}`;
    $('profitPositionCard')?.classList.toggle('loss',expected<0);
    return {rawKg,rawCost,oilKg,processCost,kholCredit,oilCostKg,recommended,expected};
  }

  function useLatestProfitBatch(){
    const latest=getCompanyBatches().slice().sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))||String(b.createdAt||'').localeCompare(String(a.createdAt||'')))[0];
    const purchases=(window.SwatiCore?.list('purchases')||[]).filter(p=>p.itemId==='oil.raw.groundnut');
    const pq=purchases.reduce((s,p)=>s+Number(p.baseQty||p.qty||0),0),pv=purchases.reduce((s,p)=>s+Number(p.amount||0),0),avg=pq>0?pv/pq:0;
    const expenses=(window.SwatiCore?.list('expenses')||[]).filter(e=>latest&&(e.context?.notes===latest.id)).reduce((s,e)=>s+Number(e.amount||0),0);
    const oilSales=getRetailSales().filter(r=>r.category==='oil'&&Number(r.qty||0)>0);
    const recent=oilSales[oilSales.length-1],recentKg=recent?oilQtyInKg(recent.qty,recent.unit):0;
    if(latest){$('profitRawKg').value=latest.inputKg||0;$('profitRawCost').value=round2(Number(latest.inputKg||0)*avg);$('profitOilKg').value=latest.oilKg||0;$('profitKholKg').value=latest.khaliKg||latest.kholKg||0;$('profitProcessCost').value=expenses;}
    $('profitKholRate').value=$('profitKholRate').value||settings.kholRate||0;
    if(recentKg>0) $('profitSellingRate').value=round2(Number(recent.total||0)/recentKg);
    profitPlannerCalc();
  }
  ...
  function retailSelectedItem(){
    if(($('retailCategory')?.value||'oil')==='oil') return 'મગફળીનું તેલ';
    const selected=$('retailGrainItem')?.value||'';
    return selected==='other'?($('retailOtherItem')?.value||'').trim():selected;
  }
  function setRetailCategoryUI(category='oil',item=''){
    const oil=category==='oil';
    $('retailCategory').value=oil?'oil':'grain';
    $('retailOilItemField').hidden=!oil;$('retailGrainItemField').hidden=oil;
    if(oil){$('retailItem').value='મગફળીનું તેલ';const unit=$('retailUnit').value;$('retailUnit').value=['kg','mann','litre','tin'].includes(unit)?unit:'kg';$('retailOtherItemField').hidden=true;}
    else{
      if(!['kg','mann'].includes($('retailUnit').value)) $('retailUnit').value='kg';
      const opts=[...$('retailGrainItem').options].map(o=>o.value);
      $('retailGrainItem').value=opts.includes(item)?item:(item?'other':'');
      $('retailOtherItem').value=opts.includes(item)?'':item;
      $('retailOtherItemField').hidden=$('retailGrainItem').value!=='other';
    }
    document.querySelectorAll('[data-retail-category]').forEach(b=>b.classList.toggle('active',b.dataset.retailCategory===(oil?'oil':'grain')));
  }
  function retailCalc(){
  ...
  function grainProductionCalc(){
    ...
    const goodKg=normalizeGrainWeight($('grainProdGoodQty')?.value,$('grainProdGoodUnit')?.value||'kg');
    const wasteKg=normalizeGrainWeight($('grainProdWasteQty')?.value,$('grainProdWasteUnit')?.value||'kg');
    const lossKg=Math.max(0,round2(inputKg-goodKg-wasteKg));
    if($('grainProdLossQty')) $('grainProdLossQty').value=lossKg;
  ...
  function grainCustomerWasteHistoryRows(){
    return getTx().filter(r=>r.business==='grain'&&r.grain?.purchaseEnabled===true&&Number(r.grain?.purchaseKg||0)>0).map(r=>({date:r.date||'',billNo:r.billNo||'',customer:r.customer?.name||'',commodity:r.grain?.commodity||'',purchaseKg:round2(Number(r.grain?.purchaseKg||0)),purchaseRate:round2(Number(r.grain?.purchaseRate||0)),purchaseAmount:round2(Number(r.grain?.purchaseAmount||0))})).sort((a,b)=>String(b.date).localeCompare(String(a.date)));
  }
  function renderGrainCustomerWasteHistory(){
    const panel=$('grainCustomerWastePanel');if(!panel)return;
    const active=($('grainSaleType')?.value||'processed')==='waste';panel.hidden=!active;if(!active)return;
    const rows=grainCustomerWasteHistoryRows(),totalKg=round2(rows.reduce((s,r)=>s+r.purchaseKg,0)),totalValue=round2(rows.reduce((s,r)=>s+r.purchaseAmount,0));
    $('grainCustomerWasteCount').textContent=String(rows.length);$('grainCustomerWasteQty').textContent=`${totalKg} kg`;$('grainCustomerWasteValue').textContent=money(totalValue);
    $('grainCustomerWasteList').innerHTML=rows.map(r=>`<div class="customer-waste-row"><span><strong>${escapeHtml(r.customer||'Customer')}</strong><small>${escapeHtml(r.date)} • ${escapeHtml(r.billNo)} • ${escapeHtml(r.commodity)}</small></span><span class="customer-waste-numbers"><strong>${r.purchaseKg} kg</strong><small>${money(r.purchaseRate)}/kg • ${money(r.purchaseAmount)}</small></span></div>`).join('')||'<div class="empty">હજુ grain job-work Waste / Reject purchase નથી.</div>';
  }
  ...
  function editRecord(id){
    ...
    const materialInput=(type,qtyId,unitId)=>{
      const line=(r.incoming?.materials||[]).find(x=>x.type===type)||{};
      $(qtyId).value=line.enteredQty ?? r.incoming?.[`${type}Qty`] ?? r.incoming?.[`${type}Kg`] ?? 0;
      $(unitId).value=line.unit||r.incoming?.[`${type}Unit`]||'kg';
    };
    materialInput('pods','peanutPodsQty','peanutPodsUnit');
    materialInput('grains','peanutGrainsQty','peanutGrainsUnit');
    materialInput('splits','peanutSplitsQty','peanutSplitsUnit');
  ...
  function deleteRecord(id){
    ...
    window.SwatiCore?.replaceStockMovements?.('grain_jobwork_waste',id,[]);
  ...
  function exportCSV(businessType){
    ...
    const header=['તારીખ','બિલ નંબર','ગ્રાહકનું નામ','મોબાઇલ નંબર','ગામ','શીંગ / Pods (કિલો)','દાણા / Grains (કિલો)','ફાડા / Splits (કિલો)','તેલના ડબા','વધારાનું તેલ (કિલો)','મજૂરી કામ','કુલ ખોળ બન્યો','મિલે ખરીદેલ ખોળ','ગ્રાહકને પરત ખોળ','ખોળનો ભાવ','ખોળ ખરીદી રકમ','Process Difference','નવા ડબા','તેલ વેચાણ રકમ','કુલ લેવાના','કુલ આપવાના','અંતિમ રકમ','દિશા','ચૂકવેલ / મળેલ','બાકી','સ્થિતિ','ડિલિવરી તારીખ','Entry By'];
    lines.push(header.join(','));
    for(const r of rows){
      const vals=[r.date,r.billNo,r.customer?.name,r.customer?.mobile,r.customer?.village,r.incoming?.podsKg||'',r.incoming?.grainsKg||'',r.incoming?.splitsKg||'',r.oilOutput?.tins||'',r.oilOutput?.extraKg||'',r.jobWorkAmount,r.khol?.producedKg||'',r.khol?.purchaseKg||'',r.khol?.customerKg||'',r.khol?.rate||'',r.khol?.amount||0,r.outputs?.lossKg||'',r.newTin?.qty||'',r.oilSale?.amount||0,r.settlement?.receivable,r.settlement?.payable,Math.abs(Number(r.settlement?.net||0)),directionText(r),paymentTotal(r),remainingFor(r),jobStatusLabel(r.status?.code),r.status?.deliveryDate||'',r.updatedBy||r.createdBy||''];
      lines.push(vals.map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(','));
    }
  ...
  function purchaseReportData(){
    if(!window.SwatiCore) return [];
    return (window.SwatiCore.list('purchases')||[]).map(r=>({
  ...
  function expenseReportData(){
    if(!window.SwatiCore) return [];
    return (window.SwatiCore.list('expenses')||[]).map(r=>({
  ...
  function batchReportData(){
    return getBatches().map(r=>({
      date:r.toDate||r.saleDate||'',title:r.batchNo||'Batch',
      meta:`${r.fromDate||'—'} થી ${r.toDate||'—'} • ખરીદી ${money(r.purchaseCost||0)} • ખોળ વેચાણ ${money(r.saleAmount||0)} • ${Number(r.profit||0)>=0?'Profit':'Loss'} ${money(Math.abs(Number(r.profit||0)))}`,
      amount:Number(r.saleAmount||0),outstanding:0,payable:Number(r.totalCost||0),profit:Number(r.profit||0)
    }));
  }
  ...
  function getExpandedReportData(type){
    ...
    if(type==='batches') return batchReportData();
  ...
  function renderExpandedReport(){
    ...
    else if(activeExpandedReport==='batches') fourthLabel='Total Cost';
  ...
  function renderReportVisuals(){
    const salesRows=salesReportData(),sales=round2(salesRows.reduce((s,r)=>s+Number(r.amount||0),0));
    const job=round2(getTx().reduce((s,r)=>s+Number(r.jobWorkAmount||0),0));
    const expenses=round2(expenseReportData().reduce((s,r)=>s+Number(r.amount||0),0));
    const receivable=round2(salesRows.reduce((s,r)=>s+Number(r.outstanding||0),0)+getTx().filter(r=>r.settlement?.net>0).reduce((s,r)=>s+remainingFor(r),0));
    const revenue=round2(sales+job),collected=Math.max(0,revenue-receivable),collectionPct=revenue>0?Math.max(0,Math.min(100,Math.round(collected/revenue*100))):0;
    const max=Math.max(sales,job,expenses,1),height=v=>`${Math.max(8,Math.round(Number(v||0)/max*100))}%`;
    if($('analysisSalesValue')) $('analysisSalesValue').textContent=money(sales);if($('analysisJobValue')) $('analysisJobValue').textContent=money(job);if($('analysisExpenseValue')) $('analysisExpenseValue').textContent=money(expenses);
    if($('analysisSalesBar')) $('analysisSalesBar').style.height=height(sales);if($('analysisJobBar')) $('analysisJobBar').style.height=height(job);if($('analysisExpenseBar')) $('analysisExpenseBar').style.height=height(expenses);
    if($('analysisNetPosition')) $('analysisNetPosition').textContent=money(revenue-expenses);
    if($('analysisCollectionPct')) $('analysisCollectionPct').textContent=`${collectionPct}%`;if($('analysisReceivable')) $('analysisReceivable').textContent=`${money(receivable)} બાકી`;
    if($('analysisDonut')) $('analysisDonut').style.background=`conic-gradient(var(--alpha40-green) ${collectionPct}%,#e5ebf2 0)`;
    const stock=companyStock(),values=[Math.max(0,stock.rawAvailable),Math.max(0,stock.oilAvailableKg),Math.max(0,stock.khaliAvailable)],stockMax=Math.max(...values,1);
    [['analysisRawStock','analysisRawStockBar',values[0]],['analysisOilStock','analysisOilStockBar',values[1]],['analysisKholStock','analysisKholStockBar',values[2]]].forEach(([valueId,barId,value])=>{if($(valueId)) $(valueId).textContent=round2(value);if($(barId)) $(barId).style.width=`${Math.max(3,Math.round(value/stockMax*100))}%`;});
  }
  ...
  function exportBackup(){
    ...
    const payload={version:41,exportedAt:new Date().toISOString(),currentOperator:currentOperator(),deviceId:deviceId(),data};
  ...
  const SCREEN_TITLES={
    ...
    'company-sales':'તેલ / ખોળ વેચાણ',
    'company-stock':'કંપની સ્ટોક',
    purchases:'ખરીદી અને ખર્ચ',
    customers:'ગ્રાહકો',
    'retail-sales':'રિટેલ વેચાણ',
    history:'હિસ્ટ્રી',
    stock:'Batch',
    usage:'વપરાશ',
    expenses:'ખરીદી અને ખર્ચ',
    'stock-management':'Stock સુધારો',
    costing:'ભાવ અને નફો',
    finance:'ફાઇનાન્સ',
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    if(initial&&$(`screen-${initial}`)) showScreenInternal(initial);
  });
  ...
  function billShareText(r,business){
    ...
    if(business!=='oil') return `*સ્વાતિ મિની ઓઇલ મિલ*\n_${b} મજૂરી બિલ_\n\n*બિલ:* ${r?.billNo||'—'}  |  *તારીખ:* ${r?.date||'—'}\n*ગ્રાહક:* ${r?.customer?.name||'—'}\n*ગામ:* ${r?.customer?.village||'—'}\n\n*મજૂરી:* ${money(r?.jobWorkAmount||0)}\n*અંતિમ હિસાબ:* ${money(Math.abs(Number(r?.settlement?.net||0)))}\n*ચૂકવેલ / મળેલ:* ${money(paymentTotal(r))}\n*બાકી:* ${money(remainingFor(r))}\n\nઆભાર 🙏`;
    const lines=[
      `*ખોળ:* ${r?.khol?.purchaseKg||0} kg × ${money(r?.khol?.rate||0)} = ${money(r?.khol?.amount||0)}`,
      `*મજૂરી:* ${oilOutputLabel(r?.oilOutput?.tins||0,r?.oilOutput?.extraKg||0)} × ${money(r?.rates?.jobRatePerTin||settings.jobRatePerTin)}/ડબો = ${money(r?.jobWorkAmount||0)}`
    ];
    if(Number(r?.newTin?.qty||0)>0) lines.push(`*નવું ડબો:* ${r.newTin.qty} × ${money(r.newTin.rate||0)} = ${money(r.newTin.amount||0)}`);
    if(r?.oilSale?.enabled&&Number(r?.oilSale?.qty||0)>0) lines.push(`*તેલ વેચાણ:* ${r.oilSale.qty} ${r.oilSale.unit} × ${money(r.oilSale.rate||0)} = ${money(r.oilSale.amount||0)}`);
    const settlementLabel=Number(r?.settlement?.net||0)>0?'ખેડૂત પાસેથી લેવાના':Number(r?.settlement?.net||0)<0?'ખેડૂતને આપવાના':'સરભર પૂર્ણ';
    return `*સ્વાતિ મિની ઓઇલ મિલ*\n_તેલ મિલ મજૂરી બિલ_\n\n*બિલ:* ${r?.billNo||'—'}  |  *તારીખ:* ${r?.date||'—'}\n*ગ્રાહક:* ${r?.customer?.name||'—'}\n*ગામ:* ${r?.customer?.village||'—'}\n\n${lines.join('\n')}\n\n*${settlementLabel}:* ${money(Math.abs(Number(r?.settlement?.net||0)))}\n*ચૂકવેલ / મળેલ:* ${money(paymentTotal(r))}\n*બાકી:* ${money(remainingFor(r))}\n*સ્થિતિ:* ${jobStatusLabel(r?.status?.code)}${r?.status?.deliveryDate?`\n*ડિલિવરી:* ${r.status.deliveryDate}`:''}\n\nઆભાર 🙏`;
  }
  ...
  ['rawQtyKg','rawRateKg','rawKgPerBag','rawTransportCost','rawBrokerCost','rawOtherCost'].forEach(id=>$(id)?.addEventListener('input',()=>rawPurchaseCalc(true)));
  $('rawQtyUnit')?.addEventListener('change',()=>rawPurchaseCalc(true));
  $('rawPaid')?.addEventListener('input',()=>rawPurchaseCalc(false));
  ['productionInputKg','productionTinCount','productionOilLooseKg','productionKholKg'].forEach(id=>$(id)?.addEventListener('input',productionCalc));
  ['productionInputUnit','productionKholUnit'].forEach(id=>$(id)?.addEventListener('change',productionCalc));
  ['companySaleTinCount','companySaleKg','companySaleRate','companySalePaid','companySaleTransport','companySaleLoading','companySaleUnloading'].forEach(id=>$(id)?.addEventListener('input',companySaleCalc));
  $('companySaleCustomer')?.addEventListener('change',()=>{const name=$('companySaleCustomer').value.trim().toLowerCase(),party=getCompanySales().slice().reverse().find(x=>String(x.customer||'').trim().toLowerCase()===name);if(party){if(!$('companySaleVillage').value)$('companySaleVillage').value=party.village||'';if(!$('companySaleMobile').value)$('companySaleMobile').value=party.mobile||'';}});
  ...
  $('rawPurchaseForm')?.addEventListener('submit',(e)=>{
    e.preventDefault();
    const c=rawPurchaseCalc(),rate=Number($('rawRateKg').value||0),paid=Number($('rawPaid').value||0),editId=$('rawPurchaseEditId')?.value||'';
    if(c.enteredQty<=0||c.qtyKg<=0){toast('જથ્થો અને kg per bag ચેક કરો');return;}
    const rows=getCompanyPurchases();
    const old=editId?rows.find(x=>x.id===editId):null;
    const rawRow={id:editId||companyUid('RAW'),date:$('rawPurchaseDate').value||todayISO(),supplier:$('rawSupplier').value.trim(),village:$('rawVillage').value.trim(),yard:$('rawYard').value.trim(),mobile:$('rawMobile').value.trim(),material:$('rawMaterial').value,quality:$('rawQuality').value.trim(),enteredQty:round2(c.enteredQty),unit:c.unit,bagCount:c.unit==='bag'?round2(c.enteredQty):0,kgPerBag:round2(c.kgPerBag),qtyKg:round2(c.qtyKg),rateKg:c.landedPerKg,ratePerUnit:round2(rate),materialTotal:c.materialTotal,transportCost:round2(c.transport),brokerCost:round2(c.broker),otherCost:round2(c.other),total:c.total,landedCostPerMann:round2(c.landedPerKg*20),landedCostPerBag:c.kgPerBag?round2(c.landedPerKg*c.kgPerBag):0,expectedOilRatio:round2(num('rawExpectedOilRatio')),expectedKholRatio:round2(num('rawExpectedKholRatio')),paid:round2(paid),outstanding:c.out,note:$('rawNote').value.trim(),createdAt:old?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString(),operator:currentOperator()};
    if(editId){const i=rows.findIndex(x=>x.id===editId);if(i>=0)rows[i]=rawRow;}else rows.push(rawRow);
    saveCompanyPurchases(rows);
    const payload={date:rawRow.date,party:rawRow.supplier,itemId:`oil.raw.${rawRow.material||'groundnut'}`,itemName:rawRow.material||'મગફળી',qty:rawRow.qtyKg,unitName:'kg',rate:rawRow.rateKg,amount:rawRow.total,paid:rawRow.paid,paymentMode:'cash',context:{division:'oil_mill',unit:'production',activity:'raw_material_purchase',sourceModule:'oil_company',operator:rawRow.operator,notes:rawRow.id}};
    const linked=editId?(window.SwatiCore?.list('purchases')||[]).find(x=>x.context?.sourceModule==='oil_company'&&x.context?.notes===editId):null;
    if(linked&&window.SwatiCore?.updatePurchase) window.SwatiCore.updatePurchase(linked.id,payload); else window.SwatiCore?.addPurchase(payload);
    resetRawPurchaseForm(); renderCompanyProduction(); toast(editId?'ખરીદી update થઈ':'કાચા માલની ખરીદી સાચવાઈ');
  });

  function resetRawPurchaseForm(){
    $('rawPurchaseForm')?.reset();$('rawPurchaseEditId').value='';$('rawPurchaseDate').value=todayISO();$('rawMaterial').value='groundnut';$('rawQtyUnit').value='kg';$('rawKgPerBag').value='20';$('rawPaid').value='0';$('rawPurchaseSaveBtn').textContent='ખરીદી સાચવો';$('rawPurchaseEditCancelBtn').hidden=true;rawPurchaseCalc();
  }
  function editRawPurchase(id){
    const r=getCompanyPurchases().find(x=>x.id===id);if(!r)return;
    $('rawPurchaseEditId').value=r.id;$('rawPurchaseDate').value=r.date||todayISO();$('rawSupplier').value=r.supplier||'';$('rawVillage').value=r.village||'';$('rawYard').value=r.yard||'';$('rawMobile').value=r.mobile||'';$('rawQuality').value=r.quality||'';$('rawMaterial').value=r.material||'groundnut';$('rawQtyKg').value=r.enteredQty||0;$('rawQtyUnit').value=r.unit||'kg';$('rawKgPerBag').value=r.kgPerBag||20;$('rawRateKg').value=r.ratePerUnit||0;$('rawTransportCost').value=r.transportCost||0;$('rawBrokerCost').value=r.brokerCost||0;$('rawOtherCost').value=r.otherCost||0;$('rawExpectedOilRatio').value=r.expectedOilRatio||0;$('rawExpectedKholRatio').value=r.expectedKholRatio||0;$('rawPaid').value=r.paid||0;$('rawNote').value=r.note||'';$('rawPurchaseSaveBtn').textContent='ખરીદી Update કરો';$('rawPurchaseEditCancelBtn').hidden=false;rawPurchaseCalc(false);$('rawPurchaseForm').scrollIntoView({behavior:'smooth',block:'start'});
  }
  $('rawPurchaseList')?.addEventListener('click',e=>{const b=e.target.closest('[data-edit-raw-purchase]');if(b)editRawPurchase(b.dataset.editRawPurchase);});
  $('rawPurchaseEditCancelBtn')?.addEventListener('click',resetRawPurchaseForm);

  $('productionBatchForm')?.addEventListener('submit',(e)=>{
    e.preventDefault();
    const c=productionCalc(),stock=companyStock(),editId=$('productionBatchEditId')?.value||'',rows=getCompanyBatches(),old=editId?rows.find(x=>x.id===editId):null;
    if(c.input<=0){toast('Input કિલો દાખલ કરો');return;}
    if(c.input>stock.rawAvailable+Number(old?.inputKg||0)+0.001){toast('Raw stock પૂરતો નથી');return;}
    if(c.oil+c.khali+c.loss<=0){toast('Output દાખલ કરો');return;}
    const emptyTinAvailable=Number(window.SwatiCore?.stockBalance?.('oil.packaging.empty_tin_15kg')||0)+Number(old?.tinCount||0);
    if(c.tinCount>emptyTinAvailable+.001){toast(`માત્ર ${Math.max(0,emptyTinAvailable)} ખાલી ડબા ઉપલબ્ધ છે`);return;}
    const prodRow={id:editId||companyUid('PR'),date:$('productionDate').value||todayISO(),batchNo:$('productionBatchNo').value||nextProductionBatchNo(),inputKg:round2(c.input),oilKg:round2(c.oil),tinCount:c.tinCount,looseOilKg:c.looseOilKg,khaliKg:round2(c.khali),lossKg:round2(c.loss),operator:$('productionOperator').value||currentOperator(),note:$('productionNote').value.trim(),oilYield:c.oilYield,khaliYield:c.khaliYield,createdAt:old?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};
    if(editId){const i=rows.findIndex(x=>x.id===editId);if(i>=0)rows[i]=prodRow;}else rows.push(prodRow);
    saveCompanyBatches(rows);
    const prodCtx={division:'oil_mill',unit:'production',activity:'production_batch',sourceModule:'oil_company',operator:prodRow.operator,notes:prodRow.id};
    window.SwatiCore?.replaceStockMovements?.('oil_production',prodRow.id,[
      {date:prodRow.date,itemId:'oil.raw.groundnut',itemName:'મગફળી',qty:prodRow.inputKg,unitName:'kg',movementType:'production_consumption',direction:'out',context:prodCtx},
      {date:prodRow.date,itemId:'oil.finished.oil',itemName:'તેલ',qty:prodRow.oilKg,unitName:'kg',movementType:'production_in',direction:'in',context:prodCtx},
      {date:prodRow.date,itemId:'oil.byproduct.khali',itemName:'ખોળ',qty:prodRow.khaliKg,unitName:'kg',movementType:'production_in',direction:'in',context:prodCtx},
      {date:prodRow.date,itemId:'oil.packaging.empty_tin_15kg',itemName:'ખાલી 15 કિલો ડબો',qty:prodRow.tinCount,unitName:'tin',movementType:'production_consumption',direction:'out',context:prodCtx},
      {date:prodRow.date,itemId:'oil.packaging.filled_tin_15kg',itemName:'15 કિલો ભરેલું ટીન',qty:prodRow.tinCount,unitName:'tin',movementType:'production_in',direction:'in',context:prodCtx}
    ]);
    resetProductionBatchForm();renderCompanyProduction();toast(editId?'પ્રોડક્શન બેચ update થયો':'પ્રોડક્શન બેચ સાચવાયો');
  });

  function resetProductionBatchForm(){$('productionBatchForm').reset();$('productionBatchEditId').value='';$('productionDate').value=todayISO();$('productionBatchNo').value=nextProductionBatchNo();$('productionLossKg').value='0';$('productionTinCount').value='0';$('productionOilLooseKg').value='0';$('productionInputUnit').value='kg';$('productionKholUnit').value='kg';$('productionBatchSaveBtn').textContent='બેચ સાચવો';$('productionBatchEditCancelBtn').hidden=true;hydrateCompanyOperator();productionCalc();}
  function editProductionBatch(id){const r=getCompanyBatches().find(x=>x.id===id);if(!r)return;$('productionBatchEditId').value=r.id;$('productionDate').value=r.date||todayISO();$('productionBatchNo').value=r.batchNo||'';$('productionInputKg').value=r.inputKg||0;$('productionInputUnit').value='kg';$('productionTinCount').value=r.tinCount??Math.floor(Number(r.oilKg||0)/15);$('productionOilLooseKg').value=r.looseOilKg??round2(Number(r.oilKg||0)-(Number($('productionTinCount').value||0)*15));$('productionKholKg').value=r.khaliKg||0;$('productionKholUnit').value='kg';$('productionOperator').value=r.operator||currentOperator();$('productionNote').value=r.note||'';$('productionBatchSaveBtn').textContent='બેચ Update કરો';$('productionBatchEditCancelBtn').hidden=false;productionCalc();$('productionBatchForm').scrollIntoView({behavior:'smooth',block:'start'});}
  $('productionBatchList')?.addEventListener('click',e=>{const b=e.target.closest('[data-edit-production-batch]');if(b)editProductionBatch(b.dataset.editProductionBatch);});
  $('productionBatchEditCancelBtn')?.addEventListener('click',resetProductionBatchForm);
  ...
  $('companySaleForm')?.addEventListener('submit',(e)=>{
    ...
    const saleRow={id:companySaleEditId||companyUid('SALE'),date:$('companySaleDate').value||todayISO(),customer:$('companySaleCustomer').value.trim(),village:$('companySaleVillage').value.trim(),mobile:$('companySaleMobile').value.trim(),channel:$('companySaleChannel').value||'retail',product:c.product,unit:c.unit,tinCount:c.product==='oil'?c.tinCount:0,enteredQty:c.enteredQty,kg:c.kg,oilKg:c.oilKg,rate:round2(Number($('companySaleRate').value||0)),transportCost:c.transport,loadingCost:c.loading,unloadingCost:c.unloading,extraCost:c.extraCost,total:c.total,net:c.net,paid:round2(Number($('companySalePaid').value||0)),outstanding:c.out,method:$('companySaleMethod').value,note:$('companySaleNote').value.trim(),createdAt:new Date().toISOString(),operator:currentOperator()};
    ...
    if(window.SwatiCore){
      const itemId=saleRow.product==='khol'?'oil.byproduct.khali':'oil.finished.oil';
    ...
    document.querySelectorAll('[data-sale-product]').forEach(b=>b.classList.toggle('active',b.dataset.saleProduct==='khol'));
    $('companySaleUnit').value='kg';$('companySaleChannel').value='retail';$('companySaleTinCount').value='0';$('companySaleKg').value='0';$('companySalePaid').value='0';
    $('companySaleTinField').hidden=true;$('companySaleKgField').hidden=false;companySaleCalc();clearEditMode('companySale','વેચાણ સાચવો');renderCompanySales();toast(companySaleEditId?'વેચાણ update થયું':'વેચાણ સાચવાયું');
  });
  ...
  ['purchaseQty','purchaseRate'].forEach(id=>$(id)?.addEventListener('input',()=>purchaseCalc(true)));
  ['purchasePaid','purchaseTransport','purchaseLoading','purchaseUnloading'].forEach(id=>$(id)?.addEventListener('input',()=>purchaseCalc(false)));
  ...
  const UI_TEXT = {
    ...
    reports:'રિપોર્ટ્સ',
      ...
      grainStockHelp:'Raw, Processed અને Waste/Reject stock જુઓ',
      grainSalesHelp:'Processed અને Waste/Reject sales માટેનું workspace.',
    ...
    en:{
      ...
      grainStockHelp:'View raw, processed and Waste/Reject stock',
  ...
  document.querySelectorAll('[data-sale-product]').forEach(btn=>btn.addEventListener('click',()=>{
    ...
    $('companySaleUnit').value=khali?'kg':'tin';$('companySaleTinField').hidden=khali;$('companySaleKgField').hidden=!khali;companySaleCalc();renderCompanySales();
  }));
  document.querySelectorAll('[data-retail-category]').forEach(btn=>btn.addEventListener('click',()=>{
    setRetailCategoryUI(btn.dataset.retailCategory);
    renderRetailSales();
  }));
  $('retailGrainItem')?.addEventListener('change',()=>{$('retailOtherItemField').hidden=$('retailGrainItem').value!=='other';});
  ['retailQty','retailRate','retailPaid'].forEach(id=>$(id)?.addEventListener('input',retailCalc));
  const saveRetailSale=(e)=>{
    e.preventDefault();const c=retailCalc(),item=retailSelectedItem();
    if(c.qty<=0){toast('જથ્થો દાખલ કરો');return;} if(!item){toast('આઇટમ દાખલ કરો');return;}
    if($('retailCategory').value==='grain'&&!['kg','mann'].includes($('retailUnit').value)){toast('અનાજ / કઠોળ માટે કિલો અથવા મણ પસંદ કરો');return;}
    ...
    const rows=getRetailSales();if(retailEditId){const i=rows.findIndex(x=>x.id===retailEditId);if(i>=0) rows[i]={...rows[i],...row,updatedAt:new Date().toISOString()};}else rows.push(row);
    if(window.SwatiCore){
      const linked=retailEditId?linkedCoreSaleBySourceId(row.id):null;
      const stockQty=oilQtyInKg(row.qty,row.unit);
      const itemId=row.category==='oil'?'oil.finished.oil':grainStockItemId(row.item,'processed');
      const available=Number(window.SwatiCore.stockBalance(itemId)||0)+(linked?.itemId===itemId?Number(linked?.baseQty||0):0);
      if(stockQty>available+.001){toast(`સ્ટોક માત્ર ${round2(available)} કિલો ઉપલબ્ધ છે`);return;}
      const salePayload={date:row.date,party:row.customer,itemId,itemName:row.item,qty:stockQty,unitName:'kg',rate:stockQty>0?round2(row.total/stockQty):0,amount:row.total,received:row.paid,paymentMode:row.paymentMode,context:{division:row.category==='oil'?'oil_mill':'grain_pulse',unit:'production',activity:'retail_sale',sourceModule:'retail_sales_alpha41',operator:row.operator,notes:row.id}};
      if(linked && window.SwatiCore.updateSale) window.SwatiCore.updateSale(linked.id,salePayload); else window.SwatiCore.addSale(salePayload);
    }
    saveRetailSales(rows);
    $('retailSaleForm').reset();$('retailSaleDate').value=todayISO();$('retailPaid').value='0';setRetailCategoryUI('oil');
    renderRetailSales();toast('રિટેલ વેચાણ સાચવાયું');
  };
  $('retailSaleForm')?.addEventListener('submit',saveRetailSale);
  ...
  ['grainProdInputQty','grainProdInputUnit','grainProdGoodQty','grainProdGoodUnit','grainProdWasteQty','grainProdWasteUnit',
  ...
  $('grainProductionForm')?.addEventListener('submit',(e)=>{
    ...
    if(row.wasteKg>0 && row.wasteItem) window.SwatiCore.addStockMovement({date:row.date,itemId:row.wasteItemId,itemName:row.wasteItem,qty:row.wasteKg,baseQty:row.wasteKg,unitName:'kg',baseUnitName:'kg',movementType:'production_in',context:{...ctx,activity:'grain_waste_output'}});
    ...
    $('grainProductionForm').reset();$('grainProdDate').value=keepDate;$('grainProdInputUnit').value='kg';$('grainProdGoodUnit').value='kg';$('grainProdWasteUnit').value='kg';
  ...
  ['profitRawKg','profitRawCost','profitOilKg','profitProcessCost','profitKholKg','profitKholRate','profitMargin','profitSellingRate'].forEach(id=>$(id)?.addEventListener('input',profitPlannerCalc));
  $('profitUseLatestBtn')?.addEventListener('click',useLatestProfitBatch);
  ...
  document.querySelectorAll('[data-stock-detail]').forEach(btn=>btn.addEventListener('click',()=>renderCompanyStockDetail(btn.dataset.stockDetail)));
  $('companyStockDetailClose')?.addEventListener('click',()=>{$('companyStockDetail').hidden=true;});
  $('companyRatioOpenBtn')?.addEventListener('click',()=>{renderCompanyRatioPanel();$('companyRatioPanel').hidden=false;$('companyRatioPanel').scrollIntoView({behavior:'smooth',block:'start'});});
  $('companyRatioCloseBtn')?.addEventListener('click',()=>{$('companyRatioPanel').hidden=true;});
  const closeMobileTools=()=>{if($('mobileToolsModal')) $('mobileToolsModal').hidden=true;};
  $('mobileToolsBtn')?.addEventListener('click',()=>{if($('mobileToolsModal')) $('mobileToolsModal').hidden=false;});
  $('mobileToolsClose')?.addEventListener('click',closeMobileTools);
  $('mobileToolsModal')?.addEventListener('click',e=>{if(e.target===$('mobileToolsModal')) closeMobileTools();});
  $('mobileToolsModal')?.querySelectorAll('[data-go]').forEach(btn=>btn.addEventListener('click',closeMobileTools));
  ...
  $('companySalesList')?.addEventListener('click',(e)=>{
    ...
    $('companySaleProduct').value=r.product||'khol';$('companySaleChannel').value=r.channel||'retail';$('companySaleUnit').value=r.unit||'kg';$('companySaleTinCount').value=r.tinCount||0;$('companySaleKg').value=r.enteredQty??(r.unit==='mann'?round2(Number(r.kg||0)/20):r.unit==='ton'?round2(Number(r.kg||0)/1000):r.kg||0);
    $('companySaleRate').value=r.rate||0;$('companySalePaid').value=r.paid||0;$('companySaleMethod').value=r.method||'cash';$('companySaleTransport').value=r.transportCost||0;$('companySaleLoading').value=r.loadingCost||0;$('companySaleUnloading').value=r.unloadingCost||0;$('companySaleNote').value=r.note||'';
    document.querySelectorAll('[data-sale-product]').forEach(b=>b.classList.toggle('active',b.dataset.saleProduct===(r.product||'khol')));
    $('companySaleTinField').hidden=r.unit!=='tin';$('companySaleKgField').hidden=r.unit==='tin';
  ...
  $('retailSalesList')?.addEventListener('click',(e)=>{
    ...
    setRetailCategoryUI(r.category||'oil',r.item||'');$('retailQty').value=r.qty||0;$('retailUnit').value=r.unit||'kg';$('retailRate').value=r.rate||0;$('retailPaid').value=r.paid||0;$('retailPaymentMode').value=r.paymentMode||'cash';$('retailNote').value=r.note||'';
    setEditMode('retail',r.id,'વેચાણ Update કરો');retailCalc();window.scrollTo({top:0,behavior:'smooth'});
  });
  $('retailEditCancelBtn')?.addEventListener('click',()=>{$('retailSaleForm')?.reset();setRetailCategoryUI('oil');clearEditMode('retail','વેચાણ સાચવો');renderRetailSales();});
